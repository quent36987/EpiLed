...
import { getshootingTargetPart } from '../utils/decoupe';

export interface IShootingTargetProps {
	frequency: number;
	colorCount: number;
	firstColor: string;
	endColor: string;
...
export function createShootingTargetAnimation(
	leds: ILed[],
	props: IShootingTargetProps
): IStepAnimation {
	...
	for (let part = 0; part <= props.partCount; part++) {
		const ledsInPart = getshootingTargetPart(
			leds,
			props.partCount,
			part,
			centerX,
...
export const SHOOTING_TARGET_MODULES: IModule[] = [
	{
		title: 'Color Count',
		name: 'colorCount',
		range: {
